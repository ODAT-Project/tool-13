...
from sklearn.inspection import permutation_importance #for RSF feature importance
from sksurv.ensemble import RandomSurvivalForest
from sksurv.util import Surv
from sksurv.metrics import concordance_index_censored #for permutation importance scorer
from lifelines import CoxPHFitter
from lifelines.utils import concordance_index #for CPH C-index
...
class DynamicCVDApp:
    RSF_RISK_SCORE_COL_NAME = 'RSF_Risk_Score_Covariate'

    def __init__(self, root):
        self.root = root
        self.root.title("Advanced Dynamic CVD Risk Predictor -- RSF-CPH Enhanced")
        ...
        self.rsf_model = None
        self.cph_model = None
        self.scaler_rsf = None
        self.scaler_cph_linear = None
        self.num_imputer_rsf = None
        self.num_imputer_cph_linear = None

        self.trained_rsf_feature_names = []
        self.trained_cph_linear_feature_names = []
        self.all_base_features_for_input = []

        self.trained_feature_medians_rsf = {}
        self.trained_feature_medians_cph_linear = {}
        self.scaled_columns_rsf = []
        self.scaled_columns_cph_linear = []

        #storing processed training data for RSF permutation importance
        self.X_rsf_processed_train = None
        self.y_rsf_survival_train = None
        ...
        self.max_depth_var = tk.StringVar(value="10") #default, can be "" or "None" for unlimited
    ...
    def create_train_config_widgets(self, parent_frame):
        ...
        rsf_fs_frame = ttk.LabelFrame(parent_frame, text="Features for RSF Risk Score Model")
        rsf_fs_frame.pack(fill=tk.BOTH, expand=True, pady=5)
        
        ttk.Label(rsf_fs_frame, text="Select features for non-linear risk score (more is often better for RSF):").pack(anchor=tk.W, padx=5, pady=(5,0))
        
        rsf_listbox_container = ttk.Frame(rsf_fs_frame, style="Content.TFrame")
        rsf_listbox_container.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        self.rsf_feature_listbox = tk.Listbox(rsf_listbox_container, selectmode=tk.MULTIPLE, exportselection=False, height=6, bg=STYLE_CONFIG["bg_entry"], fg=STYLE_CONFIG["fg_text"], selectbackground=STYLE_CONFIG["listbox_select_bg"], selectforeground=STYLE_CONFIG["listbox_select_fg"], highlightthickness=1, highlightbackground=STYLE_CONFIG["border_color"], font=(STYLE_CONFIG["font_family"], STYLE_CONFIG["font_size_normal"]))
        
        rsf_feature_listbox_scrollbar = ttk.Scrollbar(rsf_listbox_container, orient=tk.VERTICAL, command=self.rsf_feature_listbox.yview)
        self.rsf_feature_listbox.configure(yscrollcommand=rsf_feature_listbox_scrollbar.set)
        
        rsf_feature_listbox_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.rsf_feature_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        rsf_params_frame = ttk.LabelFrame(parent_frame, text="Risk Score Model (RSF) Hyperparameters")
        rsf_params_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(rsf_params_frame, text="Estimators:").grid(row=0, column=0, padx=5, pady=3, sticky=tk.W)
        
        self.n_estimators_entry = ttk.Entry(rsf_params_frame, textvariable=self.n_estimators_var, width=8, font=(STYLE_CONFIG["font_family"], STYLE_CONFIG["font_size_normal"]))
        self.n_estimators_entry.grid(row=0, column=1, padx=5, pady=3, sticky=tk.W)
        
        ttk.Label(rsf_params_frame, text="Max Depth (optional, blank for None):").grid(row=0, column=2, padx=5, pady=3, sticky=tk.W)
        
        self.max_depth_entry = ttk.Entry(rsf_params_frame, textvariable=self.max_depth_var, width=8, font=(STYLE_CONFIG["font_family"], STYLE_CONFIG["font_size_normal"]))
        ...
        self.train_button = ttk.Button(parent_frame, text="Train Hybrid RSF-CPH Model", command=self.train_model_action)
    ...
    def create_dynamic_prediction_inputs(self):
        ...
        self.prediction_input_widgets = {}
        self.all_base_features_for_input = sorted(list(set(self.trained_rsf_feature_names + self.trained_cph_linear_feature_names)))
        ...
        for feature_name in self.all_base_features_for_input:
            ...
            if feature_name in self.trained_feature_medians_rsf: default_val = self.trained_feature_medians_rsf.get(feature_name, "0")
    ...
    def create_results_display_widgets(self, parent_frame):
        ...
        self.ax_importance_rsf = self.fig.add_subplot(2, 2, 1); self.ax_cph_coeffs = self.fig.add_subplot(2, 2, 2)    
        self.ax_calibration_like = self.fig.add_subplot(2, 2, 3); self.ax_survival_curve = self.fig.add_subplot(2, 2, 4) 
        
        for ax in [self.ax_importance_rsf, self.ax_cph_coeffs, self.ax_calibration_like, self.ax_survival_curve]:
    ...
    def toggle_train_predict_sections_enabled(self, data_loaded=False, model_trained=False):
        ...
        if hasattr(self, 'rsf_feature_listbox'): self.rsf_feature_listbox.config(state=train_state) 
    ...
    def _populate_ui_lists_after_load(self, column_names):
        self.rsf_feature_listbox.delete(0, tk.END); self.cph_linear_feature_listbox.delete(0, tk.END)
        
        for col_name in column_names: self.rsf_feature_listbox.insert(tk.END, col_name); self.cph_linear_feature_listbox.insert(tk.END, col_name)
    ...
    def load_csv_file(self):
        ...
        try:
            ...
            self.rsf_model = None; self.cph_model = None; self.X_rsf_processed_train = None; self.y_rsf_survival_train = None
    ...
    def train_model_action(self):
        ...
        selected_rsf_indices = self.rsf_feature_listbox.curselection()
        ...
        if not selected_rsf_indices: messagebox.showerror("Error", "No features selected for RSF Model. Please select at least one."); return
        ...
        self.trained_rsf_feature_names = [self.rsf_feature_listbox.get(i) for i in selected_rsf_indices]
        
        self.trained_cph_linear_feature_names = [self.cph_linear_feature_listbox.get(i) for i in selected_cph_linear_indices]
        
        for col_list_name, col_list in [("RSF", self.trained_rsf_feature_names), ("CPH", self.trained_cph_linear_feature_names)]:
        ...
        if self.RSF_RISK_SCORE_COL_NAME in self.trained_cph_linear_feature_names:
             messagebox.showerror("Error", f"'{self.RSF_RISK_SCORE_COL_NAME}' is reserved."); return
        try:
            ...
            if n_estimators <= 0 or (max_depth is not None and max_depth <= 0) or cph_penalizer < 0:
        ...
        self.log_training_message(f"RSF Features ({len(self.trained_rsf_feature_names)}): {self.trained_rsf_feature_names[:5]}...")
        ...
        self.log_training_message("\n--- Stage 1: Training Risk Score Model (Random Survival Forest) ---")
        try:
            X_rsf_full = full_df_processed[self.trained_rsf_feature_names]
            
            self.scaler_rsf = StandardScaler(); self.num_imputer_rsf = SimpleImputer(strategy='median')
            
            self.trained_feature_medians_rsf = {}; self.scaled_columns_rsf = []
            
            X_rsf_processed = self._preprocess_features(X_rsf_full, self.trained_rsf_feature_names, self.trained_feature_medians_rsf, self.scaler_rsf, self.scaled_columns_rsf, self.num_imputer_rsf, fit_transform=True)
            
            self.log_training_message(f"  RSF processed features: {X_rsf_processed.shape}")
            
            self.log_training_message(f"  RSF Scaled columns: {self.scaled_columns_rsf if self.scaled_columns_rsf else 'None'}")
            ...
            y_rsf_survival = Surv.from_arrays(event=y_event_status, time=y_time_to_event)
            
            self.rsf_model = RandomSurvivalForest(n_estimators=n_estimators, max_depth=max_depth, random_state=42, n_jobs=-1)
            
            self.rsf_model.fit(X_rsf_processed, y_rsf_survival)
            
            self.log_training_message("  Random Survival Forest model trained.")
            
            self.X_rsf_processed_train = X_rsf_processed.copy(); self.y_rsf_survival_train = y_rsf_survival
            
            rsf_risk_scores_all_data = self.rsf_model.predict(X_rsf_processed)
            
            full_df_processed[self.RSF_RISK_SCORE_COL_NAME] = rsf_risk_scores_all_data
            
            self.log_training_message(f"  '{self.RSF_RISK_SCORE_COL_NAME}' generated for all {len(rsf_risk_scores_all_data)} samples.")
            
            if len(rsf_risk_scores_all_data) > 0:
                self.log_training_message(f"    Sample of '{self.RSF_RISK_SCORE_COL_NAME}' (first 5): {np.round(rsf_risk_scores_all_data[:5], 4)}")
                self.log_training_message(f"    Stats for '{self.RSF_RISK_SCORE_COL_NAME}': Min={np.min(rsf_risk_scores_all_data):.4f}, Max={np.max(rsf_risk_scores_all_data):.4f}, Mean={np.mean(rsf_risk_scores_all_data):.4f}, StdDev={np.std(rsf_risk_scores_all_data):.4f}")
        except Exception as e:
            self.log_training_message(f"Error in RSF training: {type(e).__name__}: {str(e)}", is_error=True); self.log_training_message(traceback.format_exc(), is_error=True)
            messagebox.showerror("RSF Training Error", f"Failed RSF stage ({type(e).__name__}): {e}"); self.rsf_model = None; return
        ...
        try:
            if self.trained_cph_linear_feature_names:
                ...
                df_for_cph_fitting[self.RSF_RISK_SCORE_COL_NAME] = full_df_processed[self.RSF_RISK_SCORE_COL_NAME]
            else: 
                df_for_cph_fitting = pd.DataFrame({self.RSF_RISK_SCORE_COL_NAME: full_df_processed[self.RSF_RISK_SCORE_COL_NAME]})
            ...
            self.generate_training_report(n_estimators, max_depth, cph_penalizer)
            ...
            self.toggle_train_predict_sections_enabled(data_loaded=True, model_trained=bool(self.rsf_model))
        finally:
            if 'full_df_processed' in locals(): del full_df_processed; 
            
            if 'X_rsf_full' in locals(): del X_rsf_full
            
            if 'X_rsf_processed' in locals() and self.X_rsf_processed_train is not X_rsf_processed : del X_rsf_processed
    ...
    def generate_training_report(self, n_est, max_d, cph_pen):
        ...
        report_lines.append(f"  Features used: {len(self.trained_rsf_feature_names)}")
        
        report_lines.append(f"  Hyperparameters: Estimators={n_est}, MaxDepth={max_d if max_d is not None else 'Unlimited'}")
        
        report_lines.append("\nCPH Model:")
        
        cph_features_in_model = self.trained_cph_linear_feature_names + [self.RSF_RISK_SCORE_COL_NAME]
        
        report_lines.append(f"  Features used (incl. RSF score): {len(cph_features_in_model)}")
    ...
    def assess_risk_action(self):
        if not self.rsf_model or not self.cph_model or not self.all_base_features_for_input:
        ...
        input_df_rsf_filtered = input_df_for_processing[self.trained_rsf_feature_names].copy()
        
        input_rsf_processed = self._preprocess_features(input_df_rsf_filtered, self.trained_rsf_feature_names, self.trained_feature_medians_rsf, self.scaler_rsf, self.scaled_columns_rsf, self.num_imputer_rsf, fit_transform=False)
        
        try: new_rsf_risk_score = self.rsf_model.predict(input_rsf_processed)[0]
        except Exception as e:
            self.log_training_message(f"RSF prediction error: {str(e)}", is_error=True); self.log_training_message(traceback.format_exc(), is_error=True)
            messagebox.showerror("Prediction Error", f"Could not generate RSF risk score: {e}"); return
        
        cph_input_dict = {self.RSF_RISK_SCORE_COL_NAME: new_rsf_risk_score}
    ...
    def update_plots(self, clear_only=False):
        ax_list = [self.ax_importance_rsf, self.ax_cph_coeffs, self.ax_calibration_like, self.ax_survival_curve] 
        ...
        self.ax_importance_rsf.set_title("RSF: Permutation Feature Importances", fontdict=title_font_dict)
        
        if clear_only or not self.rsf_model:
            self.ax_importance_rsf.text(0.5, 0.5, "Train RSF model first.", ha='center', va='center', color=tick_color)
        
        elif not self.trained_rsf_feature_names: #check if any features were selected for RSF
             self.ax_importance_rsf.text(0.5, 0.5, "No features were selected for RSF training.", ha='center', va='center', color=tick_color)
        
        elif self.X_rsf_processed_train is None or self.y_rsf_survival_train is None:
            self.ax_importance_rsf.text(0.5, 0.5, "RSF training data not stored\nfor permutation importance.", ha='center', va='center', color=tick_color)
        
        elif self.X_rsf_processed_train.shape[1] == 0 : #check if processed train data actually has features
             self.ax_importance_rsf.text(0.5, 0.5, "No features available in RSF\ntraining data for importance.", ha='center', va='center', color=tick_color)
        else:
            try:
                self.log_training_message("Calculating permutation importance for RSF (may take a moment)...", is_error=False)
                def sksurv_concordance_scorer(estimator, X_perm, y_structured_array_perm):
                    y_pred_risk = estimator.predict(X_perm)
                    event_indicator = y_structured_array_perm['event']
                    event_time = y_structured_array_perm['time']
                    c_index, _, _, _, _ = concordance_index_censored(event_indicator, event_time, y_pred_risk)
                    return c_index

                perm_importance_result = permutation_importance(
                    self.rsf_model, self.X_rsf_processed_train, self.y_rsf_survival_train,
                    scoring=sksurv_concordance_scorer, n_repeats=5, random_state=42, n_jobs=-1
                )
                sorted_idx = perm_importance_result.importances_mean.argsort()[::-1]
                importances_mean = perm_importance_result.importances_mean[sorted_idx]
                
                #get feature names from the columns of X_rsf_processed_train
                feature_names_for_plot = np.array(self.trained_rsf_feature_names)[sorted_idx]

                num_features_to_plot = min(len(importances_mean), 10)
                
                self.ax_importance_rsf.barh(range(num_features_to_plot), importances_mean[:num_features_to_plot][::-1], align="center", color=bar_color)
                
                self.ax_importance_rsf.set_yticks(range(num_features_to_plot))
                
                self.ax_importance_rsf.set_yticklabels(feature_names_for_plot[:num_features_to_plot][::-1], fontdict={'fontsize': STYLE_CONFIG["font_size_normal"]-2})
                
                self.ax_importance_rsf.set_xlabel("Permutation Importance (C-index drop on train set)", fontdict=label_font_dict)
                
                self.log_training_message("Permutation importance calculation complete.", is_error=False)
            except ImportError:
                 self.ax_importance_rsf.text(0.5, 0.5, "sklearn.inspection not found.\nCannot compute permutation importance.", ha='center', va='center', color=tick_color)
            except Exception as e_perm:
                self.log_training_message(f"Error calculating permutation importance: {type(e_perm).__name__} - {e_perm}", is_error=True)
                
                self.log_training_message(traceback.format_exc(), is_error=True)
                
                self.ax_importance_rsf.text(0.5, 0.5, f"Error calculating RSF importances:\n{str(e_perm)[:40]}...", ha='center', va='center', color=STYLE_CONFIG["error_text_color"])
        self.ax_importance_rsf.tick_params(axis='x', colors=tick_color); self.ax_importance_rsf.tick_params(axis='y', colors=tick_color); self.ax_importance_rsf.set_facecolor(STYLE_CONFIG["bg_entry"])
    ...
    def show_about_dialog(self):
        messagebox.showinfo("About Advanced Dynamic CVD Predictor",
                            "Advanced Dynamic CVD Predictor -- RSF-CPH Enhanced\n\n" 
                            "Random Survival Forest model generates a non-linear risk score. CPH model incorporates this score along with other linear predictors for survival analysis.\n\n" 
                            "Developed by ODAT project.")
